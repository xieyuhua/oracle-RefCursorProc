...
func initDb() error {
	// 连接配置
	dbconn, err := sql.Open("oracle", *dns)
	if err != nil {
		return fmt.Errorf("数据库连接失败: %v", err)
	}

	// 配置连接池参数
	dbconn.SetMaxOpenConns(20)           // 最大活动连接数
	dbconn.SetMaxIdleConns(5)           // 最小保持连接数（空闲连接保留数）
	dbconn.SetConnMaxLifetime(60 * time.Minute) // 连接最大生命周期
	dbconn.SetConnMaxIdleTime(5 * time.Minute)  // 空闲连接最大存活时间
...
func checkConnectionsHealth() {
	...
	if err := conn.PingContext(ctx); err != nil {
	    // 获取连接池统计信息
    	stats := conn.Stats()
    	log.Printf("连接池状态: 打开连接数=%d, 使用中=%d, 空闲=%d, 等待=%d", stats.OpenConnections, stats.InUse, stats.Idle, stats.WaitCount)
        if stats.InUse == 0 {
            conn.Close()
            initDb()
        }
    ...
    for range ticker.C {
        ...
        query := `
            ...
            WHERE status = 'INACTIVE'
              ...
              AND program LIKE '%oracle_proc%'
        `
        
        rows, err := db.Query(query)
        if err != nil {
            logs.Errorf("查询空闲会话失败: %v", err)
            return
        }
        defer rows.Close()
        ...
        for rows.Next() {
            ...
            if err := rows.Scan(&s.sid, &s.serial, &s.user); err == nil {
        ...
        for _, s := range sessions {
            killSQL := fmt.Sprintf("ALTER SYSTEM KILL SESSION '%d,%d' IMMEDIATE", s.sid, s.serial)
            _, err := db.Exec(killSQL)
            if err != nil {
                logs.Warnf("清理会话 %d,%d 失败: %v", s.sid, s.serial, err)
            } else {
                logs.Infof("已清理空闲会话: SID=%d, 用户=%s", s.sid, s.user)
            }
        }
    }
}
